...
function updateFinancialAnalysis(period) {
  ...
  switch (period) {
    case "week":
      ...
      periodText = `من ${formatDate(startDate)} إلى ${formatDate(now)}`;
      break;
    case "month":
      startDate = new Date(now);
      startDate.setMonth(now.getMonth() - 1);
      periodText = `شهر ${getMonthName(
        startDate.getMonth()
      )} سنة ${startDate.getFullYear()}`;
      break;
    case "year":
      startDate = new Date(now);
      startDate.setFullYear(now.getFullYear() - 1);
      periodText = `سنة ${startDate.getFullYear()}`;
      break;
    default:
  ...
  // Update the period text display
